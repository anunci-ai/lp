  ...
  return (
    <section className="relative py-28 px-5 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="glass-card rounded-2xl p-6 flex flex-col gap-5 relative overflow-hidden"
            >
              {/* Gradient glow in corner */}
              <div
                className={`absolute -top-10 -right-10 w-32 h-32 rounded-full opacity-10 bg-gradient-to-br ${t.gradient}`}
                style={{ filter: "blur(30px)" }}
              />

              {/* Quote mark */}
              <div className={`text-5xl font-black leading-none bg-gradient-to-br ${t.gradient}`}
                style={{
              ...
              <div className="flex gap-1">
                {[...Array(t.stars)].map((_, j) => (
                  <svg key={j} width="14" height="14" viewBox="0 0 14 14" fill="#FFB800">
        ...
        >
          {[
            { icon: "⭐", label: "4.9/5 média de avaliações" },
            { icon: "🔒", label: "Dados protegidos" },
            { icon: "🇧🇷", label: "Feito para o mercado brasileiro" },
          ].map((item) => (
            <div key={item.label} className="flex items-center gap-2.5 text-white/40 text-sm">
